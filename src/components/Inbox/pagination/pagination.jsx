const Pagination = ({
  ...
  const buttonAvailable = totalPages > 1 ? true : false;
  const handlePrevious = () => {
    if (currentPage > 1) {
      router.push(`/opinions?page=${currentPage - 1}`);
    }
  };
  const handleNext = () => {
    ...
    <div className="flex items-center justify-between border-t border-stroke p-4 dark:border-strokedark sm:px-6">
      ...
      {buttonAvailable && (
        <div className="flex items-center justify-end space-x-3">
          <button
            onClick={prevPage}
            className="flex h-7.5 w-7.5 items-center justify-center rounded border border-stroke bg-whiten hover:border-primary hover:bg-primary hover:text-white dark:border-strokedark dark:bg-whiten/30"
          >
            <svg
              width="18"
              height="18"
